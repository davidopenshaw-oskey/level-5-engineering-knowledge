...
// (Phase 1.75) -- Kotlin/Android port. Same generic technique as
// 06-build-cross-module-dependency-graph.ts, scoped to submodule-to-
// submodule coupling WITHIN one module. Real, non-trivial for exactly one
// of this repo's 5 modules: `app`, whose 11 real screens (Home, Digicode,
// CallRTC, etc -- see Task 3's own scan-repo output) are Task 3's own real
// submodule partition. The 4 library modules get uniform submodule=null
// (Task 3's own decision -- real file counts too small to warrant
// partitioning), so this script honestly reports zero submodule coupling
// for those, same as node-iot's own RBAC section correctly, honestly
// stayed empty for a repo with no RBAC code -- not a sign this script is
// broken for those modules.

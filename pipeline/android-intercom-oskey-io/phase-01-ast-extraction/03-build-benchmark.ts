...
// Kotlin/Android port. Validates every module's own evidence-graph/manifest
// identity and record counts against the repo-wide AST manifest, then builds
// a single-source-of-truth repo-wide totals report. Real, deliberate
// omission vs. the TS pipelines' own copy: no `recommendationMethod` POC-
// complexity-scoring section -- that weighting (service/controller/mongo-
// Operation counts) is specific to a classification scheme this repo
// deliberately hasn't decided yet (config/repos.json's own classification
// Rules is left unset, see 02-p1-build-tasklist.md Task 2's note) -- adding
// a scoring heuristic without a real classification scheme to weight would
// be inventing signal, not reporting it.
...
function main() {
  ...
  if (!fs.existsSync(astManifestPath)) {
    addNotification(notifications, SOURCE_SCRIPT, "fatal", "MISSING_AST_MANIFEST_FATAL", `Missing required ast-evidence-manifest.json at '${astManifestPath}'.`);

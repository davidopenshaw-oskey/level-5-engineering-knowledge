...
//
// Shared run-notification, atomic-write, and path-safety utilities for this
// repo's own pipeline scripts (00 onward). Ported from angular-app-oskey-io's
// copy of this file, deliberately scoped down: that copy has accumulated a
// large block of Phase-2/LLM-citation-restoration helpers (short-ID citation
// handling, footnote rendering) that belong to LLM-synthesis output
// validation, not Phase 1 extraction -- not carried over here. Add them back
// only when this repo's own Phase 2 actually needs them, not preemptively.
//
// SCOPE NOTE: this module is intentionally scoped to THIS repo's pipeline
// only. Per the multi-repo architecture decision, each repo owns its own
// fully independent pipeline and may freely duplicate or reimplement this
// file locally -- no cross-repo shared runtime.
...
/**
 * Resolves the namespaced run-context path for a given repo. Namespacing by
 * repoName means each repo's pipeline output subtree is self-contained and
 * cannot collide with another repo's run, even if runs happen to overlap.
 */
 ...
 * resolve to a specific repo, especially with multiple repo pipelines

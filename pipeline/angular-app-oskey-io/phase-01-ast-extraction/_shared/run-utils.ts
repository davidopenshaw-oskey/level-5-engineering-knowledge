...
export function factsToCompactTable(facts: any[]): string {
  ...
  // finding 2026-08-30 on firebase-oskey-dev (governance/roadmap/
  // firebase-oskey-dev/09-fact-table-redundancy-reduction.md), ported here
  // verbatim -- domain-agnostic, operates purely on whichever columns are
  // actually constant in the input, not hardcoded to Firebase's fact shape.
  // module/repo/runId were confirmed 100% constant across all facts in a
  // real whole-module test there, costing 8.5% of the entire table for zero
  // new information. Detected dynamically (not hardcoded to assume these 3
  // are always constant) so this stays correct if a future caller ever
  // combines facts across modules/runs -- in that case nothing is hoisted
  // and every column still renders per-row exactly as before. `type` is
  // handled separately below: it's always constant WITHIN a section by
  // construction (that's what defines the section), and already stated once
  // via the "## <type> (<count>)" header immediately above every row that
 ...
 * Ported verbatim from firebase-oskey-dev's copy of this file, real,
 * measured finding 2026-08-30 there (governance/roadmap/firebase-oskey-dev/
 ...
 * fact ID (`id`, e.g. `angular_component|features|...|OSKFooComponent`)
 * remains the one stable, durable identity for a fact everywhere else in
 * this pipeline (evidence graphs, provenance sidecars, any future
 * fact-to-workflow indexing) -- this short reference only ever exists for
 * the lifetime of one prompt/response round-trip, never written to disk on
 * its own. */
...
// space, backtick, F123, backtick, space, backtick-backtick. A first
// version of this regex assumed no spacing and no inner backticks and
// silently matched nothing real, found 2026-08-30 during a real end-to-end
// test on firebase-oskey-dev.
// Second alternative added 2026-08-31 (ported from firebase-oskey-dev's
// copy, found there by node-iot's own gemini-3.7-flash test): the model
// sometimes drops the outer double-backtick entirely, leaving a bare
// single-backtick wrap (`` `F967` ``) instead of the expected doubled form.
// Worse than the bundled-citation bug above -- invisible to BOTH this
// pattern's old doubled-only form AND UNRESTORED_SHORT_ID_PATTERN's old
// doubled-only form, so it would produce dangling, unverifiable references
// while the run's own citation count still reported clean. Checked this
// repo's own real captured module-level responses properly before porting
// (normalized away every already-known malformation first via
// expandShortIdRangeCitations/expandBundledShortIdCitations, then searched
// what remained for a genuinely bare single-backtick shape) -- confirmed
// zero real instances here, ported anyway per this session's own
// established discipline: a real, confirmed code gap gets fixed regardless
// of whether the sample happened to trigger it.
...
// Real, recurring pattern found 2026-08-30 on firebase-oskey-dev across a
// full 12-module real batch: 20 malformed range citations (`` `F150-F157` ``
// etc.) across its two largest modules -- not a rare one-off, scales with
// real output volume/citation count. The contract explicitly forbids this,
// but a model shortcut for citing several adjacent facts at once recurs
// anyway. Rather than only detecting it (findUnrestoredShortIdCitations
// already does that), expand it into one real, resolvable citation per fact
// in the range -- more citations than the model intended to write, but
// every one of them verifiable, which is strictly better than a silent,
// unverifiable range surviving into the final text.
// Inner single-backtick wrapper made optional 2026-08-31 (ported from
// firebase-oskey-dev's copy, same-day fix there): a real temp=0.4 variance
// run on `organization` produced a NEW malformed form never seen at
// temp=0.0/0.2 -- `` F493-F496 `` with no inner backtick wrapper at all,
// which the previously-mandatory-inner-backtick pattern didn't match, so it
// silently fell through as "unrestored" instead of being expanded. Matches
// the same optional-wrapper tolerance style FACT_ID_CITATION_BLOCK_PATTERN
// and FILE_LINE_CITATION_BLOCK_PATTERN already use elsewhere in this file.
const SHORT_ID_RANGE_PATTERN = /`{2}\s*`?F(\d+)-F(\d+)`?\s*`{2}/g;
// A range this large is far outside anything observed in real Firebase data
// (the biggest real one was 25 facts) -- almost certainly a different kind
// of model error, not a genuine "cite this whole contiguous block"
// shortcut. Left unexpanded (and thus still caught by
// findUnrestoredShortIdCitations) rather than silently generating an
// enormous citation list.
...
// Found 2026-08-31 on gemini-3.7-flash (76 real instances in this repo's
// own 'core' run2; zero on gemini-3.5-flash at any temperature tested, and
// zero in the OTHER gemini-3.7-flash run against the same facts --
// genuinely intermittent, not universal to the model): multiple short-ID
// citations sharing ONE outer double-backtick wrapper instead of each
// getting its own -- `` `F145` `F189` `` instead of `` `F145` `` `` `F189` ``.
// Confirmed completely invisible to every existing safety net before this
// fix: SHORT_FACT_ID_PATTERN requires exactly one inner token between the
// outer backticks, so it doesn't match this shape at all --
// restoreFactIdCitations left it as raw, unresolved "F145"/"F189" literal
// text; findUnrestoredShortIdCitations didn't match this shape either;
// citation-validator.ts's extractCitations found zero citations in it. The
// run's own citation-validation count was a silent undercount when this
// occurred, not a loud failure -- worse than the range-citation bug above,
// which at least surfaced as "unrestored".
//
// Ported from firebase-oskey-dev's copy initially with a stricter pattern
// (`` /`{2}\s*((?:`F\d+`\s*){2,})`{2}/g `` -- requiring the inner content to
// be an exact back-to-back repetition of `` `F\d+` `` chunks), then
// LOOSENED after that stricter version failed to match this repo's own real
// captured instance: the actual bytes in gemini-3.7-flash-run2's output were
// `` `F145` ` `F189` `` -- an extra, unpaired stray backtick sitting between
// the two tokens that the strict repeating-group pattern doesn't tolerate.
// Firebase's own captured case happened to be cleaner (no stray backtick),
// so their pattern worked for their sample but not for this repo's real
// data -- confirmed by testing the ported pattern against the exact real
// bytes before committing to it, not assumed to transfer. Current version
// captures everything between one `` boundary and the next (tolerating any
// internal junk, stray backticks included) and only acts if 2+ real F\d+
// tokens are found inside -- otherwise returns the match unchanged, so a
// well-formed single citation, two separate adjacent citations, an
// unrelated double-backtick code span, or a real (non-short-ID) fact-ID
// citation are all left untouched. Verified against all of those cases plus
// both the clean and stray-backtick malformed shapes before shipping.
const BUNDLED_SHORT_ID_PATTERN = /`{2}((?:`(?!`)|[^`])*)`{2}/g;

/** Expands a malformed multi-citation bundle (several short IDs sharing one
 * outer wrapper, however messy the content between them) into one
 * well-formed, individually-wrapped citation per ID -- call this BEFORE
 * restoreFactIdCitations, same as expandShortIdRangeCitations, so every
 * expanded reference gets resolved the normal way. Returns the match
 * unchanged whenever fewer than 2 short-ID tokens are found inside, so this
 * is a safe no-op for anything that isn't a genuine bundle. */
...
export function restoreFactIdCitations(text: string, idMap: Record<string, string>): string {
  // Two capture groups now (doubled-wrapper form, bare-single-backtick
  // form) are mutually exclusive per match -- only one is ever defined,
  // matching SHORT_FACT_ID_PATTERN's alternation above.
...
// on firebase-oskey-dev: restoreFactIdCitations left it completely
// untouched, AND it's invisible to citation-validator.ts too (its
// FACT_ID_PATTERN requires content to start with a known fact-type name,
// which "F201-F203" doesn't) -- so a malformed short-ID citation silently
// disappears from evidence tracking entirely rather than being flagged as
// fabricated or unverified. This is a visibility check, not a fix: run it
// AFTER restoreFactIdCitations and treat any match as a real problem worth
// surfacing (a notification, a thrown error, or at minimum a console
// warning), not something to silently tolerate. Widened alongside
// SHORT_FACT_ID_PATTERN above (ported from firebase-oskey-dev's copy) --
// a single-backtick-only leftover (`` `F967` ``, no outer double-backtick)
// was invisible to the old `{2}`-only version of this safety net too, same
// as it was invisible to restoration itself. This is the LAST line of
// defense if restoration somehow still misses a future shape -- deliberately
// kept looser than the restoration patterns on purpose (matches either the
// double-outer form or a bare single-backtick wrapper), so a genuinely new
// malformation shape still gets flagged loudly even if no restoration fix
// for it exists yet.
...
//
// Real, user-driven finding 2026-08-30 on firebase-oskey-dev: a document
// with dense inline fact-ID citations (this pipeline's real, current
// convention -- see citation-validator.ts's own header comment) is
// dramatically harder to read than an old reference document that used no
// inline citations at all, just Confirmed/Inferred tags. Removing citations
// isn't the answer -- that document's claims aren't independently
// checkable, which defeats the entire point of citation-validator.ts and
// the generate-then-verify discipline this pipeline is built around. The
// fix: keep every citation fully real and checkable, but move the verbose
// fact-ID/file-line text out of the readable prose into a numbered evidence
// appendix, same convention as an academic paper's footnotes/endnotes --
// "(FactId:#001)" inline, the real citation content listed once at the end.
// Applies uniformly to BOTH citation sources in an assembled module-level
// document: the LLM's own text (already real fact IDs by the time this
// runs, after restoreFactIdCitations) and the deterministically-assembled
// sections (buildPublicInterfacesSection etc.), which write real fact IDs
// directly with no short-ID indirection at all. Ported verbatim from
// firebase-oskey-dev's copy -- domain-agnostic text rewriting.
...
// directly (``realId``) with no inner backtick or padding at all. Fact-type
// alternation kept in sync with citation-validator.ts's own FACT_ID_PATTERN
// -- both were missing Angular's own fact types until 2026-08-30, confirmed
// as a LIVE gap (not just theoretical) against a real assembled
// module-level document: all 17 real angular_injectable/angular_guard-type
// citations in it were silently invisible to citation-validator.ts's
// extractCitations(), and would have been equally invisible to this
// function's footnote extraction. Now includes Angular's real fact types,
// confirmed against 02-build-module-evidence.ts (angular_component/
// angular_injectable are assigned via a `factType` variable there, not a
// literal string, so a naive grep for `type: "..."` misses them -- checked
// the actual code path before trusting this list). `function_declaration`
// is a separate, real, ALSO-missing fact type observed in real output --
// NOT added here, since it's inherited from firebase-oskey-dev's original
// pattern too (not Angular-specific) and wasn't part of this task's scope.
const FACT_ID_CITATION_BLOCK_PATTERN =
  /`{2}\s*`?((?:model_property|api_contract|call_expression|imports_dependency|firestore_path_touched|firestore_trigger|permission_candidate|permission_error|external_hook|pubsub_topic|pubsub_publish_call|pubsub_event_route|source_class|service_method|controller_method|class_method|type_alias|enum_declaration|source_file|exported_symbol|angular_component|angular_injectable|angular_route|angular_guard|angular_signal|angular_template_composition|angular_template_binding|firebase_callable_call)\|[^`]+)`?\s*`{2}/g;
...
// single-backtick level). Real bug found 2026-08-30 on firebase-oskey-dev:
// an earlier version of this pattern assumed the outer double-backtick
// (when present) always closes BEFORE the "(lines N-M)" parenthetical --
// the real, observed structure is `` `file.ts` (line 37) `` with the outer
// close coming AFTER the parenthetical instead, leaving a dangling "``"
// artifact in the rendered document when the old pattern only matched part
// of it. The inner single-backtick-wrapped file path is always mandatory
// (matching citation-validator's own convention exactly); the outer
// double-backtick wrapper is optional on EITHER side independently, since
// which side (if any) an LLM adds it on isn't something to assume.
...
/** Renders the appendix as the document's real Evidence References section
 * content -- a real, complete list, not a promissory "see inline citations
 * above" note. Section number is the caller's concern (Firebase's is
 * Section 14; Angular's module-level document's number, once that contract
 * exists, may differ -- this function just renders the list). */
 ...
 * paragraphs instead of a dense block -- ported verbatim from
 * firebase-oskey-dev's copy, a real, user-requested readability fix there,
 * measured negligible cost repo-wide. Deliberately does NOT touch INDENTED
 * sub-bullets (e.g. a per-class method/interface enumeration), which are a

...
function main() {
  ...
  const angularDecoratorsFact = loadFactFile("ast-angular-decorators.json");
  const firebaseCallableCallsFact = loadFactFile("ast-firebase-callable-calls.json");
  const angularRoutesFact = loadFactFile("ast-angular-routes.json");
  const angularGuardsFact = loadFactFile("ast-angular-guards.json");
  ...
  for (const moduleName of authoritativeModules) {
    ...
    // 16. angular_component / angular_injectable
    for (const item of angularDecoratorsFact.filter(d => d.module === moduleName)) {
      const factType = item.decoratorType === "Component" ? "angular_component" : "angular_injectable";
      rawModuleFacts.push({
        id: stableFactId({ type: factType, repo: REPO_NAME, module: moduleName, file: item.path, line: item.line, primaryKey: item.className }),
        runId,
        type: factType,
        repo: REPO_NAME,
        module: moduleName,
    ...
    // 17. firebase_callable_call
    for (const item of firebaseCallableCallsFact.filter(c => c.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({
          type: "firebase_callable_call", repo: REPO_NAME, module: moduleName, file: item.path, line: item.line,
          primaryKey: item.functionName,
          occurrenceOrdinal: nextOccurrenceOrdinal(occurrenceCounters, "firebase_callable_call", item.path, item.functionName),
        }),
        runId, type: "firebase_callable_call", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.path, line: item.line, value: item.functionName,
        evidence: { ...item },
      });
    }

    // 18. angular_route
    for (const item of angularRoutesFact.filter(r => r.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({
          type: "angular_route", repo: REPO_NAME, module: moduleName, file: item.path, line: item.line,
          primaryKey: item.routePath,
          occurrenceOrdinal: nextOccurrenceOrdinal(occurrenceCounters, "angular_route", item.path, item.routePath),
        }),
        runId, type: "angular_route", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.path, line: item.line, value: item.routePath,
        evidence: { ...item },
      });
    }

    // 19. angular_guard
    for (const item of angularGuardsFact.filter(g => g.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({ type: "angular_guard", repo: REPO_NAME, module: moduleName, file: item.path, line: item.line, primaryKey: item.guardName }),
        runId, type: "angular_guard", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.path, line: item.line, value: item.guardName,
        evidence: { ...item },
      });
    }

    // 20. angular_signal
    for (const item of angularSignalsFact.filter(s => s.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({ type: "angular_signal", repo: REPO_NAME, module: moduleName, file: item.path, line: item.line, primaryKey: item.className, secondaryKey: item.propertyName }),
        runId, type: "angular_signal", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.path, line: item.line, value: `${item.className}.${item.propertyName}`,
        evidence: { ...item },
      });
    }

    // 21. angular_template_composition
    for (const item of angularTemplateCompositionFact.filter(t => t.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({
          type: "angular_template_composition", repo: REPO_NAME, module: moduleName, file: item.templatePath, line: item.templateLine,
          primaryKey: item.className,
          secondaryKey: item.elementTag,
          occurrenceOrdinal: nextOccurrenceOrdinal(occurrenceCounters, "angular_template_composition", item.templatePath, item.className, item.elementTag),
        }),
        runId, type: "angular_template_composition", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.templatePath, line: item.templateLine, value: `${item.className} -> ${item.elementTag}`,
        evidence: { ...item },
      });
    }

    // 22. angular_template_binding
    for (const item of angularTemplateBindingsFact.filter(t => t.module === moduleName)) {
      rawModuleFacts.push({
        id: stableFactId({
          type: "angular_template_binding", repo: REPO_NAME, module: moduleName, file: item.templatePath, line: item.templateLine,
          primaryKey: item.className,
          secondaryKey: item.bindingName,
          occurrenceOrdinal: nextOccurrenceOrdinal(occurrenceCounters, "angular_template_binding", item.templatePath, item.className, item.bindingName),
        }),
        runId, type: "angular_template_binding", repo: REPO_NAME, module: moduleName, submodule: item.submodule,
        file: item.templatePath, line: item.templateLine, value: `${item.elementTag}.${item.bindingKind}(${item.bindingName})`,
        evidence: { ...item },
      });
    }
    ...
    const summaryCounts = {
      ...
      angularComponents: facts.filter(f => f.type === "angular_component").length,
      angularInjectables: facts.filter(f => f.type === "angular_injectable").length,
      firebaseCallableCalls: facts.filter(f => f.type === "firebase_callable_call").length,
      angularRoutes: facts.filter(f => f.type === "angular_route").length,
      angularGuards: facts.filter(f => f.type === "angular_guard").length,

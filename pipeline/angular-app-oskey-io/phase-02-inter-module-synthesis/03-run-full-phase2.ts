...
// LLM call per module) as of 2026-08-31's port of firebase-oskey-dev's own
// module-level cutover (governance/roadmap/firebase-oskey-dev/10-module-
// level-production-cutover-plan.md) -- scoped first by a real per-module
// token feasibility check (relayed 2026-08-30): components (~21K est.
// tokens) and core (~90K) fit comfortably under 01e's own 700K safety
// threshold, features (~1.67M) does not, even before this port's own
// encoding fixes are counted. The old chain (01a-generate-capability-
// syntheses.ts + 01c-generate-assembly-first-profile.ts) is kept as a
// deliberate per-module fallback, not deleted: 01e has a loud-failure size
// check (MODULE_TOO_LARGE_FOR_SINGLE_CALL) for any module whose estimated
// prompt size exceeds that threshold, since batching an oversized module's
// capabilities across multiple calls is an explicitly deferred design, not
// something this script silently attempts. When 01e fails with that
// specific notification code for a module (checked via that module's
...
// script (see package.json's own "pipeline:angular") is enough. Phase 2's
...
// firebase-oskey-dev's own copy hit during its cutover testing, see
    ...
    try {
      ...
      console.log(
        `\n'${moduleName}' exceeded 01e's single-call size threshold (MODULE_TOO_LARGE_FOR_SINGLE_CALL) -- falling back to 01a+01c for this module only ` +
          `(deferred capability-batching design, same as firebase-oskey-dev's own equivalent fallback).`
      );

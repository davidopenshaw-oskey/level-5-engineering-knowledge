...
// script actually extracts. This repo's module profile has 16 sections
// (0-15), not Firebase's 15 (0-14) -- the extra one is UI Composition (5),
// which shifts Risks & Open Questions from Firebase's 13 to 14 here.
...
async function main() {
  ...
  // Firebase's equivalent script fails closed on a missing rbac-roles.json
  // because Firebase always configures one; this repo's contracts have
  // explicitly and consistently stated no such document exists yet, so
  // failing closed here would block every real run for a reason this repo
  // was never expected to satisfy. See contracts/02-repo-synthesis-reduce
  // .md's "What You're Given" note on the same point.) ---
  ...
  // (confirmed to happen: one real run of this exact call produced a
  ...
  // "Impact" bullets and had nothing left to add, rather than writing

...
// Every module-level Phase 2 run to date (this repo and angular-app-
// oskey-io's own first full run, 2026-08-29) has been done by hand -- one
// manual command per module for 01a, one per module for 01c, one repo-wide
// call for 02 -- because no single script chained them. Phase 1 doesn't
// need this: every Phase 1 script (00-07) processes the whole repo in one
// invocation, so a flat `&&`-chained npm script (see package.json's own
// "pipeline:firebase") is enough. Phase 2 is different -- 01a and 01c each
// take a MODULE_NAME and must be invoked once per module -- and the module
// list isn't fixed at author time (it comes from this run's own
// facts/modules.json, verified dynamic behavior confirmed by 01a/01c's own
// module-list validation), so a static npm-script chain can't express it
// without hardcoding a list that silently goes stale the moment a module is
// added, renamed, or removed. This script reads the live module list and
// loops.
//
...
// one of them fails.
//
// Identical logic to firebase-oskey-dev's/angular-app-oskey-io's own copy of
// this script -- fully generic (parameterized entirely by REPO_NAME/
// LLM_CONFIG_KEY), kept duplicated per repo rather than shared, matching
// this codebase's existing convention for every other Phase 1/2 script (see
// _shared/run-utils.ts's own documented scope note). This repo has exactly
// one module, so the per-module loops below run exactly once each -- no
// special-casing needed for that; the loop already handles N=1 correctly.

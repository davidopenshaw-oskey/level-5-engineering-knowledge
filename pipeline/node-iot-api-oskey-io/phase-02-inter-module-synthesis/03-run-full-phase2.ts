...
// LLM call per module) as of the cutover documented in governance/roadmap/
// node-iot-api-oskey-io/01-phase2-contract-design.md's module-level
// architecture section -- ported from firebase-oskey-dev's own production
// cutover (governance/roadmap/firebase-oskey-dev/10-module-level-
// production-cutover-plan.md, real ~68% token reduction and ~10-15x
// wall-clock speedup measured there). The old chain (01a-generate-
// capability-syntheses.ts + 01c-generate-assembly-first-profile.ts) is kept
// as a deliberate per-module fallback, not deleted: 01e has a loud-failure
// size check (MODULE_TOO_LARGE_FOR_SINGLE_CALL) for any module whose
// estimated prompt size exceeds a safety threshold, since batching an
// oversized module's capabilities across multiple calls is an explicitly
// deferred design, not something this script silently attempts. Real
// feasibility numbers for this repo's own one module (governance/roadmap/
// node-iot-api-oskey-io/01-phase2-contract-design.md, ~166K-200K estimated
// tokens) show this fallback won't trigger today -- kept anyway as
// future-proofing, matching the proven pattern rather than a shortcut, in
// case a large module is ever added. When 01e fails with that specific
// notification code for a module (checked via that module's
// run-notifications.json entry, by code+module+timestamp -- not by exit
// code alone, since exit code doesn't distinguish this case from any other
// failure), this script falls back to running 01a then 01c for that module
// only, then continues. Any other 01e failure is fail-fast, same as every
// other stage.
//
...
// script (see package.json's own "pipeline:node-iot") is enough. Phase 2's
...
//
// Identical logic to firebase-oskey-dev's own copy of this script -- fully
// generic (parameterized entirely by REPO_NAME/LLM_CONFIG_KEY), kept
...
// own documented scope note). This repo has exactly one module, so the
// per-module loop below runs exactly once -- the loop already handles N=1
...
// the same ID and be mistaken for a fresh recurrence (real false alarm
// Firebase's own testing hit, see governance/roadmap/firebase-oskey-dev/
// 10-module-level-production-cutover-plan.md).
    ...
    try {
      ...
      console.log(
        `\n'${moduleName}' exceeded 01e's single-call size threshold (MODULE_TOO_LARGE_FOR_SINGLE_CALL) -- falling back to 01a+01c for this module only ` +
          `(deferred capability-batching design).`
      );

...
async function main() {
  ...
  // silently overwrote a capability-synthesis file that belonged to a canonical
  // baseline while regenerating just one capability under a different LLM_CONFIG_KEY
  // for a comparison run (confirmed on firebase-oskey-dev's own copy). Without this,
  // a single-capability regen under any non-canonical config always corrupts the
  // canonical baseline it's meant to be compared against, rather than writing
  // alongside it.
  ...
  // See 01a-generate-capability-syntheses.ts's identical load for why this
  // must be the whole module's facts, not just this one capability pack's.

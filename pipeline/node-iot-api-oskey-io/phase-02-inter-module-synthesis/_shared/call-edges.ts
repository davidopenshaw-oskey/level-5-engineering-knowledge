...
// wiring gap pattern as the RBAC catalog (not ported here, this repo's
// reduce contract has no cross-cutting judgment slot for it -- see
// governance/roadmap/node-iot-api-oskey-io/01-phase2-contract-design.md's
// V1-A/V1-B port scoping). Module-filtered here so it can be supplied as one
// more free, already-computed input to Section 13 (Architectural
// Observations). NOT a no-op for this repo despite having only one module --
// confirmed directly against real data: this array isn't cross-module-only
// (it fires whenever a call can't be resolved to a unique target at all,
// module count aside), and this repo's own real run has one genuine entry
// (pubsub.service.ts's this.pubSub.createTopic).

...
Only relationships directly supported by evidence. This section is for genuine *other-module* dependencies only — intra-module, cross-submodule coupling (see the `@oskey/<name>/<submodule>` note in Section 5) belongs there instead, not here. For `imports_dependency` facts, infer the likely target module from the import path (e.g. a path containing `.../modules/user/...` implies a dependency on `user`) — but only name a module confirmed to exist in the live module list you were given. If a dependency appears to cross a repository boundary, name the likely target repository and explicitly note the interface definition is pending analysis of that repository.

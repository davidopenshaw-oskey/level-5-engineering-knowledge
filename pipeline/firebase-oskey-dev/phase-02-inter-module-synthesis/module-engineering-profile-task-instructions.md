...
**An Intra-Module Coupling Graph is provided in the task message — use it directly for cross-submodule coupling, do not reconstruct it yourself from raw `imports_dependency` facts.** It's deterministic (derived from AST import resolution, not inference), and covers exactly the case that used to be missed: `@oskey/building/door` is a *sibling submodule* of `building`, not a different module, even though it uses the same `@oskey/<name>` package-alias pattern as a real cross-module import like `@oskey/user` — a search that only flags imports pointing at *other top-level modules* misses this (confirmed: this exact miss happened with `building`'s door submodule in an earlier pass, before this graph existed). Report every entry from the graph as **Confirmed**, not Inferred.
...
**When multiple submodules/modules touch the same collection, use the Data Ownership Hints section provided in the task message to inform (not replace) your judgment about which one is the true owner versus a fan-out consumer.** It's a deterministic signal (how many other submodules/modules call into a given class), not an automated label — a naming-convention or single-signal heuristic can be confidently wrong, so do not report it as Confirmed ownership on its own. Combine it with the class's own evidenced Firestore paths and tag your ownership conclusion **Inferred** unless the evidence is unambiguous.

### 7. API Endpoints
Use `api_contract` facts. `requestType`/`responseType` on these facts are full type expressions (e.g. `import("...").OSKBuildingGetAllRequestData`, sometimes generic-wrapped), not expanded field lists — this is deliberate (see ADR-002, facts stay raw).

**A "Resolved API Request/Response Schemas" section is provided in the task message — use it directly, do not re-derive the join yourself.** It's a deterministic cross-reference of every `api_contract` requestType/responseType against `model_property` facts, computed once (see `_shared/api-schema-resolver.ts`), not narrative. Render the fields it gives you as the JSON block for that endpoint. If a given endpoint's type isn't listed in that section at all, no `model_property` facts matched anywhere in this module's evidence — say so explicitly rather than presenting the bare type expression as if it were a full schema.

...
**You are not given the raw evidence graph.** Every specific claim (a method name, a Firestore path, a permission string, a confidence tag) must come from what a capability output already says — you are assembling and reconciling, not re-deriving from scratch. If a capability output already hedged something as Inferred or Unknown, preserve that; do not upgrade its confidence because the claim now appears in a more polished document.
...
1. **Inbound coupling.** Each capability output only reports its own *outbound* coupling (per `contracts/00-capability-synthesis.md`'s Coupling section) — it cannot see who depends on *it*. You can, because you have every capability's output at once. When capability A's Outbound Coupling names capability B (same module, sibling submodule) or a target in another module, treat that as evidence of a relationship in both directions: A depends on B, and B is depended-on by A — even though B's own output never mentions A. Use this to correctly populate Section 5's intra-module coupling note and Section 10 (Cross-Module Relationships) — this is exactly the mechanism that closes the `@oskey/building/door` intra-module coupling gap that motivated this whole design (see `governance/adrs/adr-003.md`).

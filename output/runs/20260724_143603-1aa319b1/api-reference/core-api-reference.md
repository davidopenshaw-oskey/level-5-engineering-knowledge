...
- **Generated At**: 2026-07-24T14:36:09.842Z
...
| :--- | :--- | :--- |
| `OSKSupportedLanguageEnum` | `en_US = en_US`, `fr_FR = fr_FR` | `functions/src/modules/core/models/shared/supported_language.model.ts` |
| `OSKPincodeType` | `Inhabitant = inhabitant`, `Anonymous = anonymous`, `Guest = guest`, `Supplier = supplier`, `PermanentGuest = permanentGuest` | `functions/src/modules/core/modules/access/models/access_pincode.model.ts` |
| `OSKAccessMessageOperation` | `Insert = insert`, `Update = update`, `Delete = delete` | `functions/src/modules/core/modules/access/models/functions/access_messages.model.ts` |
| `OSKApiName` | `APNSDevelopmentAPIKey = APNS_DEV_API_KEY`, `APNSProductionAPIKey = APNS_PROD_API_KEY`, `MailerSend = MAILERSEND`, `MailtrapPassApiKey = MAILTRAP_API_TOKEN`, `MailtrapUser = MAILTRAP_USER`, `MailtrapPass = MAILTRAP_PASS`, `PubSubServiceAccountEmail = PUB_SUB_SERVICE_ACCOUNT_EMAIL`, `PubSubServiceAccountKey = PUB_SUB_SERVICE_ACCOUNT_KEY`, `OctopushPassApiKey = OCTOPUSH_API_TOKEN`, `Auth0M2MClientId = AUTH0_M2M_CLIENT_ID`, `Auth0M2MClientSecret = AUTH0_M2M_CLIENT_SECRET`, `Auth0Domain = AUTH0_DOMAIN`, `Auth0ManagementDomain = AUTH0_MANAGEMENT_DOMAIN`, `Auth0Audience = AUTH0_AUDIENCE`, `TwilioAccountSID = TWILIO_ACCOUNT_SID`, `TwilioAuthToken = TWILIO_AUTH_TOKEN`, `TwilioMessagingServiceSID = TWILIO_MESSAGING_SERVICE_SID`, `TwilioVerifyServiceID = TWILIO_VERIFY_SERVICE_ID` | `functions/src/modules/core/services/secret.service.ts` |

...
2. **Find a strong anchor, then walk outward.** Look for a confident, clearly relevant fact_id among the `search_facts` results, then call `walk_cluster` on it to trace the real chain outward in both directions. Use `get_graph_neighbors` when you only need to confirm a single hop rather than the full outward chain.

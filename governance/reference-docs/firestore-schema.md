...
**⚠ FLAGGED AS LIKELY STALE (2026-09-02):** this file was generated by walking real *live* Firestore data on 2026-06-01 — a data-derived snapshot, not a copy of a static config file (unlike `firestore.rules.txt`/`firestore.indexes.json` in this same directory, both confirmed byte-identical to the real current files as of 2026-09-02). At ~3 months old, and given this repo alone saw 13 real merges in a 2-month window around that period, real drift is likely. One concrete example already found: this doc records `/accessControlDevices/{id}/configs`, while the pipeline's own code-derived facts (`firestore_path_touched`, from `pipeline/facts-postgres-index/`) capture the real parameter name from current source: `accessControlDevices/{accessControlDeviceId}/configs`. Still listed as a required architectural grounding doc for the Phase 2 LLM synthesis pipeline (`config/repos.json`) — left in place and not removed, since removing it would break that pipeline's `readRequiredFile` fail-closed check; not currently touched. See `governance/roadmap/facts-serving-strategy/05-tasklist.md` for the real investigation this flag comes from, and its own task list for regenerating schema info if this pipeline needs current grounding again.

...
- **Correction to the speculation above**: it was not a contract fix. The user copied the actual 2026-08-11 Gemini run into `knowledge-corpus/llm-comparatives/gemini-default/` for direct comparison, and a `git diff 8601f4e..HEAD` against `contracts/` and `config/llm-providers.json` confirms **zero changes** to either between the two runs — same contracts, same config, same model, same commit. Full analysis: [`20260828 gemini-default run-to-run variance (2026-08-11 vs 2026-08-27).md`](../../knowledge-corpus/llm-comparatives/20260828%20gemini-default%20run-to-run%20variance%20(2026-08-11%20vs%202026-08-27).md).
- **The real explanation is run-to-run generation variance** (`temperature: 0.2`, not 0, on the reduce/connective calls). Citation counts — a mechanically-extracted, validated depth proxy, not eyeballed — swung by double digits in both directions across all 12 modules on this identical input: `core` +86% (66→123), `supplier` +65%, `apps` +51%, down to `unit_management` -29%, `organization` -15%. `access_control_device` only moved +10%. Verification rates stayed comparably high in both runs (97-100%) — this is a depth swing, not a quality collapse.
- **The mechanism is visible in the document structure, not just the citation count.** Old run's `core` profile groups its content into six generic buckets ("Firestore Document Orchestration," "Secret & Cryptographic Key Management," etc.) that never name the `access` capability directly. New run's `core` profile has six subsections specific to `access`'s actual internals and names five distinct service classes — matching almost exactly the "~5 distinct services" the original handover speculated existed, from citation evidence alone, without ever surfacing them as named sections. Same capability-synthesis input both times; the difference is entirely in how the reduce step organized it.
- **Practical effect on urgency**: the fact-count asymmetry across packs (743/521, and the wider table above) is real and structural, unaffected by any of this. But a single run's density numbers are not strong enough evidence on their own that the partitioning mechanism is the dominant cause of any one module reading thin, when run-to-run variance produces swings of comparable or larger size on modules that were never flagged. A decision to invest in the size-cap fix should probably wait for either multiple runs averaged together, or a different, run-independent signal — not a repeat of the original single-run comparison.
